...
export class MessageTruncator {
  ...
  static split(message, options = {}) {
    ...
    while (remaining.length > 0) {
      let rawSlice = remaining.substring(0, maxPartLength);
      let currentPart = rawSlice;
      
      // Try to break at a natural boundary
      if (remaining.length > maxPartLength) {
        rawSlice = this.truncateAtNaturalBoundary(remaining, maxPartLength);
        currentPart = rawSlice;
      }
      ...
      remaining = remaining.substring(rawSlice.length);
      partNumber++;
    }
