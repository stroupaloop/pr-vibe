...
export class ReportBuilder {
  ...
  addDecision(comment, decision, userAction, conversationData = null) {
    ...
    case 'defer':
      ...
      // Include issue information if created
      if (decision.issueUrl) {
        detailEntry.issue = {
          url: decision.issueUrl,
  ...
  addHumanReview(comment, action, analysis) {
    if (!this.report.summary.humans) {
      this.report.summary.humans = {
        total: 0,
        addressed: 0,
        tracked: 0,
        acknowledged: 0
    ...
    if (!this.report.detailedActions.humanReviews) {
      this.report.detailedActions.humanReviews = [];
    }

    const author = comment.user?.login || comment.author?.login || 'Unknown';

    // Update counts
    this.report.summary.humans.total++;
    this.report.summary.humans[action]++;

    // Add to detailed actions
    this.report.detailedActions.humanReviews.push({
      author,
      comment: {
        id: comment.id,
        body: comment.body,
  ...
  toMarkdown() {
    ...
    if (report.metadata.prUrl) {
      markdown += `🔗 [View Pull Request](${report.metadata.prUrl})\n`;
    }
    markdown += '\n';
    ...
    // Bot Approval Summary
    if (report.summary.botApprovals && Object.keys(report.summary.botApprovals).length > 0) {
      markdown += '### Bot Review Summary\n';
      for (const [botName, approval] of Object.entries(report.summary.botApprovals)) {
        const icon = approval.approved ? '✅' : '❌';
        markdown += `- **${botName}**: ${icon} ${approval.approved ? 'Approved' : 'Changes Requested'}`;
        if (approval.summary) {
          const s = approval.summary;
          const parts = [];
          if (s.mustFix > 0) parts.push(`${s.mustFix} must-fix`);
          if (s.suggestions > 0) parts.push(`${s.suggestions} suggestions`);
          if (s.nitpicks > 0) parts.push(`${s.nitpicks} nitpicks`);
          if (parts.length > 0) {
            markdown += ` (${parts.join(', ')})`;
          }
        }
    ...
    if (report.summary.humans && report.summary.humans.total > 0) {
      markdown += `- **Human Reviews**: ${report.summary.humans.total} (${report.summary.humans.addressed} addressed, ${report.summary.humans.tracked} tracked)\n`;
    }
    ...
    if (report.detailedActions.deferred.length > 0) {
      markdown += `### 📝 Deferred to Backlog (${report.detailedActions.deferred.length})\n`;
      report.detailedActions.deferred.forEach((item, idx) => {
        markdown += `${idx + 1}. **${item.bot}** - \`${item.comment.path || 'general'}${item.comment.line ? ':' + item.comment.line : ''}\`\n`;
        markdown += `   - ${item.comment.body.substring(0, 150)}...\n`;
        if (item.issue) {
          markdown += `   - **Issue Created**: [#${item.issue.number}](${item.issue.url})\n`;
        } else {
          markdown += `   - **Status**: Added to backlog\n`;
        }
        markdown += '\n';
      });
    }
    ...
    if (report.detailedActions.humanReviews && report.detailedActions.humanReviews.length > 0) {
      markdown += `### 👥 Human Reviews (${report.detailedActions.humanReviews.length})\n`;
      markdown += '*Feedback from human reviewers:*\n\n';

      const groupedByAction = report.detailedActions.humanReviews.reduce((groups, review) => {
        const action = review.action;
        if (!groups[action]) groups[action] = [];
        groups[action].push(review);
        return groups;
      }, {});

      Object.entries(groupedByAction).forEach(([action, reviews]) => {
        const actionIcon = action === 'addressed' ? '✅' : action === 'tracked' ? '📝' : '💬';
        markdown += `**${actionIcon} ${action.charAt(0).toUpperCase() + action.slice(1)} (${reviews.length})**\n\n`;

        reviews.forEach((review, idx) => {
          markdown += `${idx + 1}. **${review.author}** - \`${review.comment.path || 'general'}${review.comment.line ? ':' + review.comment.line : ''}\`\n`;
          markdown += `   "${review.comment.body.substring(0, 150)}..."\n`;
          if (review.analysis.isActionable) {
            markdown += `   - **Priority**: ${review.analysis.priority}\n`;
            markdown += `   - **Category**: ${review.analysis.category}\n`;
          }
          markdown += '\n';
        });
      });
    ...
    // Non-Critical Suggestions section
    if (report.detailedActions.nonCritical && report.detailedActions.nonCritical.length > 0) {
      markdown += `### 💡 Non-Critical Suggestions (${report.detailedActions.nonCritical.length})\n`;
      markdown += '*These are optional improvements that the bots suggested:*\n\n';
      report.detailedActions.nonCritical.forEach((item, idx) => {
        markdown += `${idx + 1}. **${item.bot}** - \`${item.comment.path || 'general'}${item.comment.line ? ':' + item.comment.line : ''}\`\n`;
        markdown += `   - ${item.comment.body.substring(0, 150)}...\n`;
        markdown += `   - Priority: ${item.decision.priority}\n`;
        if (item.decision.category) {
          markdown += `   - Category: ${item.decision.category}\n`;
        }
        markdown += '\n';
      });
    }
