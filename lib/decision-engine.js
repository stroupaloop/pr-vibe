  ...
  for (const pattern of PATTERNS.SECURITY.CRITICAL) {
    if (pattern.test(body)) {
      return {
        action: 'AUTO_FIX',
        severity: 'CRITICAL',
        reason: 'Critical security vulnerability must be fixed immediately',
        suggestedFix: generateSecurityFix(body, path)
      };
    }
  }
...
const sanitized = input.trim().replace(/[^a-zA-Z0-9]/g, '');`;
  ...
  return '// TODO: Implement fix based on specific issue';
}
