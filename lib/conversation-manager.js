...
export class ConversationManager {
  ...
  async startConversation(prNumber, comment, initialResponse) {
    ...
    const result = await this.waitForBotResponse(conversationId);
  ...
  async waitForBotResponse(conversationId) {
    ...
    while (Date.now() - startTime < this.botResponseTimeout) {
      ...
      if (newComments.length > 0) {
        ...
        if (nextAction.waitingForBot) {
          ...
          conversation.rateLimited = true;
          this.metrics.rateLimitsEncountered++;
        ...
        if (nextAction.resolved) {
          ...
          if (nextAction.resolution.includes('accepted')) {
            this.metrics.resolutionTypes.accepted++;
          } else if (nextAction.resolution.includes('Escalated')) {
            this.metrics.resolutionTypes.escalated++;
          }
        ...
        } else if (conversation.rounds.length >= this.maxRounds) {
          // Max rounds reached
          conversation.status = 'max_rounds';
          conversation.resolution = 'Maximum conversation rounds reached';
          this.metrics.resolutionTypes.maxRounds++;
          return conversation;
        }
      }
    ...
    conversation.resolution = 'Bot did not respond within timeout period';
    this.metrics.resolutionTypes.timeout++;
  ...
  async analyzeBotResponse(botResponse, conversation) {
    ...
    if (body.includes('actually') || 
      ...
      conversation.hadCorrection = true;
      this.metrics.botCorrections++;
      return {
        resolved: false,
        needsReply: true,
