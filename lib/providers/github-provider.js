...
export class GitHubProvider extends BaseProvider {
  ...
  async getReviews(prId) {
    ...
    // IMPORTANT: Process ALL reviews, even those without body text
    // Many bots (like CodeRabbit) create reviews with no body but containing nested comments
    const reviewsWithComments = await Promise.all(reviews.map(async (review) => {
      try {
        ...
        // Log debug info if there are comments in a review without body
        if (!review.body && comments.length > 0) {
          console.debug(`Found ${comments.length} comments in review ${review.id} from ${review.user.login} with no body (state: ${review.state})`);
        }
        ...
        console.debug(`Failed to fetch comments for review ${review.id}: ${error.message}`);
        // Return the review without comments if we can't fetch them
        return {
          id: review.id,
          author: review.user.login,
          body: review.body || '',
